-- Tüm egzersizleri siler ve tek seferde temiz seed atar (ikili/üçlü kayıt kalmaz).
-- Çalıştırma: mysql -u user -p VERITABANI_ADI < backend/database/reset_and_seed_exercises.sql
-- Önce exercises tablosunda is_premium kolonu olmalı (yoksa: migrate_exercises_premium.sql çalıştırın).

SET FOREIGN_KEY_CHECKS = 0;

-- 1) Egzersize bağlı tabloları temizle (FK nedeniyle önce bunlar)
DELETE FROM workout_session_exercises;
DELETE FROM workout_plan_exercises;

-- 2) Tüm egzersizleri sil
DELETE FROM exercises;

SET FOREIGN_KEY_CHECKS = 1;

-- 3) Tek seferde temiz seed (video_url / image_url veritabanında; uygulama sunucudan çeker)
-- Strength = Full Body, Arm, Chest, Shoulders - Back, Legs, Abs. Cardio / Yoga / Pilates = düz liste.
...
-- Bitti. Veritabanında her egzersiz tek kayıt; video_url ve image_url yolları sunucudan çekilir.
